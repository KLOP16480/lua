...
getgenv()['Check_Setting'] = function(Name)
    if not _G.Dis and not isfolder('น้องนับตัง HUB BF Premium') then
        makefolder('น้องนับตัง HUB BF Premium')
    end
    if not _G.Dis and not isfile('น้องนับตัง HUB BF Premium/'..Name..'.json') then
        writefile('น้องนับตัง HUB BF Premium/'..Name..'.json',JsonEncode(_G.Setting_table))
    end
end
getgenv()['Get_Setting'] = function(Name)
    if not _G.Dis and isfolder('น้องนับตัง HUB BF Premium') and isfile('น้องนับตัง HUB BF Premium/'..Name..'.json') then
        _G.Setting_table = JsonDecode(readfile('น้องนับตัง HUB BF Premium/'..Name..'.json'))
...
game.StarterGui:SetCore("SendNotification", {
	Title = "น้องนับตัง HUBน้องนับตัง HUB", 
	Text = "น้องนับตัง HUB\n",
	Icon = "http://www.roblox.com/asset/?id=9610159123",
	Duration = 8000
})

local win = library:Window("น้องนับตัง",[[Hub]],[[Version : Leak  ]],"9606070311",Enum.KeyCode.RightControl)
